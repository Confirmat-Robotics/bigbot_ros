...
enum {		M1FORWARD = 0,
			...
			SETM1ENCCOUNT = 22,
			SETM2ENCCOUNT = 23,
			GETMBATT = 24,  //interesting
			GETLBATT = 25,  //interesting
			SETMINLB = 26,
			SETMAXLB = 27,
			SETM1PID = 28,
			SETM2PID = 29,
			...
			GETBUFFERS = 47,
			GETPWMS = 48,   // interesting
			GETCURRENTS = 49,  //interesting
			MIXEDSPEED2ACCEL = 50,
			...
			SETCONFIG = 98,
			GETCONFIG = 99,
			GETAVERAGESPEEDS = 108,  //edha added
			GETSPEEDERRORS = 111, //edha added
			GETPOSERRORS = 114, //edha added
			SETM1MAXCURRENT = 133,
			SETM2MAXCURRENT = 134,
			GETM1MAXCURRENT = 135,
			GETM2MAXCURRENT = 136,
...
static void decode_read_double_4bytes(uint8_t *buffer, int32_t *enc1, int32_t *enc2);
static void decode_read_double_2bytes(uint8_t *buffer, int16_t *cur1, int16_t *cur2);
...
static int encode_read(uint8_t* buffer, uint8_t address, uint16_t *cmd_crc16, uint8_t NAME)
{
	uint8_t bytes=0;
	buffer[bytes++]=address;
	buffer[bytes++]=NAME;
...
static void decode_read_double_2bytes(uint8_t *buffer, int16_t *cur1, int16_t *cur2)
{
	*cur1=decode_uint16(buffer);
	*cur2=decode_uint16(buffer + sizeof(*cur1));
}

static void decode_read_double_4bytes(uint8_t *buffer, int32_t *enc1, int32_t *enc2)
{
	*enc1=decode_uint32(buffer);
	*enc2=decode_uint32(buffer + sizeof(*enc1));
}
...
int roboclaw_main_battery_voltage(struct roboclaw *rc,uint8_t address, int16_t *voltage)
{
	...
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETMBATT);

	if( (ret=send_cmd_wait_answer(rc, bytes, REPLY_BYTES, sent_crc16)) < 0 ) 
		return ret; //IO error or retries exceeded
		
	*voltage=decode_uint16(rc->buffer+bytes);
	// eigenlijk voltage nog door 10 delen voor float 
	return ROBOCLAW_OK;
}

int roboclaw_logic_battery_voltage(struct roboclaw *rc,uint8_t address, int16_t *voltage)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 4;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETLBATT);
...
int roboclaw_temperature(struct roboclaw *rc,uint8_t address, int16_t *temp)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 4;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETTEMP);

	if( (ret=send_cmd_wait_answer(rc, bytes, REPLY_BYTES, sent_crc16)) < 0 ) 
		return ret; //IO error or retries exceeded
		
	*temp=decode_uint16(rc->buffer+bytes);
	// eigenlijk temp nog door 10 delen voor float 
	return ROBOCLAW_OK;
}
int roboclaw_temperature_alt(struct roboclaw *rc,uint8_t address, int16_t *temp)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 4;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETTEMP2);
...
int roboclaw_encoders(struct roboclaw *rc, uint8_t address, int32_t *enc_m1, int32_t *enc_m2)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 10;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETENCODERS);

	if( (ret=send_cmd_wait_answer(rc, bytes, REPLY_BYTES, sent_crc16)) < 0 ) 
		return ret; //IO error or retries exceeded
		
	decode_read_double_4bytes(rc->buffer+bytes, enc_m1, enc_m2);

	return ROBOCLAW_OK;
}

int roboclaw_speeds(struct roboclaw *rc, uint8_t address, int32_t *sp_m1, int32_t *sp_m2)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 10;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETAVERAGESPEEDS);

	if( (ret=send_cmd_wait_answer(rc, bytes, REPLY_BYTES, sent_crc16)) < 0 )  
		return ret; //IO error or retries exceeded
		
	decode_read_double_4bytes(rc->buffer+bytes, sp_m1, sp_m2);

	return ROBOCLAW_OK;
}

int roboclaw_currents(struct roboclaw *rc, uint8_t address, int16_t *cur_m1, int16_t *cur_m2)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 6;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETCURRENTS);

	if( (ret=send_cmd_wait_answer(rc, bytes, REPLY_BYTES, sent_crc16)) < 0 ) 
		return ret; //IO error or retries exceeded
		
	decode_read_double_2bytes(rc->buffer+bytes, cur_m1, cur_m2);
	// currents nog te delen door 100 voor A
	return ROBOCLAW_OK;
}

int roboclaw_dutycycles(struct roboclaw *rc, uint8_t address, int16_t *dc_m1, int16_t *dc_m2)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 6;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETPWMS);

	if( (ret=send_cmd_wait_answer(rc, bytes, REPLY_BYTES, sent_crc16)) < 0 ) 
		return ret; //IO error or retries exceeded
		
	decode_read_double_2bytes(rc->buffer+bytes, dc_m1, dc_m2);
	// dc nog te delen door 327.67 voor %
	return ROBOCLAW_OK;
}

int roboclaw_poserrors(struct roboclaw *rc, uint8_t address, int32_t *enc_m1, int32_t *enc_m2)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 10;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETPOSERRORS);
...
int roboclaw_speederrors(struct roboclaw *rc, uint8_t address, int32_t *enc_m1, int32_t *enc_m2)
{
	int bytes, ret;
	uint16_t sent_crc16; 
	const int REPLY_BYTES = 10;
	bytes=encode_read(rc->buffer, address, &sent_crc16, GETSPEEDERRORS);
